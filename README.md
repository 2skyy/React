...
영어로 side effect는 부작용을 의미합니다. 일반적으로 프로그래밍에서 사이트 이펙트는 개발자가 의도하지 않은 코드가 실행되면서 버그각 발생하는 것을 말합니다.
...
첫번째 규칙은 무조건 최상의 레벨에서만 호출해야된다는것입니다.

여기서 최상위는 컴포넌트 의 최상위 레벨을 의미합니다.

따라서 반복문이나 조건문 또는 중처된 함수들 안에서 훅을 호출하면 안됩니다.

이 규칙에 따라서 휵은 컴포너늩가 렌더링 될때마다 같은 순서로 호출되야합니다.
